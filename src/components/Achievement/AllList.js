...
const AllList = props => {
  const {Honors, Certifications} = props;

  const returnedHonors = props.Honors.map (item => {
    return (
  ...
  const returnedCertifications = Certifications.map (item => {
    return (
      <React.Fragment key={item.ID}>
        <div className="card text-center" style={{maxWidth: '335.672px'}}>
          <img
        ...
        {((+item.ID) + (Honors.length%3)) % 3 === 0 ? <p className="w-100" /> : null}
      </React.Fragment>
    );
  });
